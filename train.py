...
dataset = load_dataset("SaffalPoosh/VITON-HD-test")
            ...
            for elem in v:
                ...
                for template in annotation_list:
                    for tag in elem["tag_info"]:
                        if (
                            tag["tag_name"] == template and tag["tag_category"] is not None
                        ):
...
def train(unet, pipe, train_dataloader, criterion, optimizer, accelerator, args, epoch):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        with torch.cuda.amp.autocast():
            ...
            prompt_embeds, negative_prompt_embeds, pooled_prompt_embeds, negative_pooled_prompt_embeds  = prompt_out
            ...
            loss = criterion(outputs[0], (batch['image'] + 1.0) / 2.0)
            ...
            if step % 100 == 0:
                logger.info(f"Epoch [{epoch}/{args.num_epochs}], Step [{step}/{len(train_dataloader)}], Loss: {loss.item():.4f}")
                
    return loss.item()


def calculate_ssim(img1, img2):
    img1 = img1.cpu().numpy().transpose(1, 2, 0) # Convert to H*W*C format
...
def validate(unet, pipe, test_dataloader, accelerator, args):
    ...
    with torch.no_grad():
        for sample in test_dataloader:
            ...
            images = pipe(
                ...
                width=args.width, 
                guidance_scale=args.guidance_scale, 
                ip_adapter_image=image_embeds,)[0]

            for i in range(len(images)):
                x_sample = pil_to_tensor(images[i])
                torchvision.utils.save_image(x_sample, os.path.join(args.output_dir, sample['im_name'][i]))
                
                ssim_score = calculate_ssim(sample['image'][i], x_sample)
                ssim_scores.append(ssim_score)

                real_images_list.append(sample['image'][i].cpu())
                generated_images_list.append(x_sample.cpu())
    ...
    fid_score = calculate_fid(real_images_tensor, generated_images_tensor, accelerator.device)
...
def main():
    ...
    for epoch in range(args.num_epochs):
        loss = train(unet, pipe, train_dataloader, criterion, optimizer, accelerator, epoch)
        validate(unet, pipe, test_dataloader, args, accelerator)
